...
export const getNearestWarehouse = async (provinceId, userLat, userLong) => {
  try {
    const response = await axios.get(
      `http://localhost:8000/api/warehouse-address/nearest-warehouse?provinceId=${provinceId}&userLat=${userLat}&userLong=${userLong}`,
    )
...
export const getShippingCost = async (origin, destination, weight, courier) => {
  try {
    console.log('warehou', origin)
          console.log('selecadd', destination)
          console.log('courier', weight, courier);
